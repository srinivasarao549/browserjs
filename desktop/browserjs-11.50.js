// 
/**
...
(function(opera){
	...
	var bjsversion=' Opera Desktop 11.50 core 2.9.168, March 8, 2012. Active patches: 210 ';
...
// TinyMCE double IFRAME init problem, some versions
	...
	if((hostname.indexOf('tokyo.jp')>-1)||(hostname.indexOf('lg.jp')>-1)){			// PATCH-186, tokyo.jp, lg.jp enable maps
	...
	} else if(hostname.indexOf(".americanexpress.")>-1){			// PATCH-446, Fix missing whitespace removal from script type attribute
		(function(sa){
			...
			if(self==top)postError.call(opera, 'Opera has modified the JavaScript on '+hostname+' (Fix missing whitespace removal from script type attribute). See browser.js for details');
		...
		if(hostname.indexOf('.mail.yahoo.')>-1&& ( href.indexOf( '/neo/launch' )>-1 || href.indexOf( '/dc/launch' )>-1 )){			// PATCH-411, remove class that adds generated content for IFRAME, triggers bug that prevents layout updates
			(function(tmp){
			...
			document.addEventListener('load', function(e){
				...
				if(self==top)postError.call(opera, 'Opera has modified the JavaScript on '+hostname+' (remove class that adds generated content for IFRAME, triggers bug that prevents layout updates\nY!Ma...). See browser.js for details');
		}
	...
	} else if(hostname.indexOf('.youtube.com')>-1){			// PATCH-447, Fix rendering of HTML5 YouTube videos
		...
		document.addEventListener('DOMContentLoaded', function() {
			...
			if(self==top)postError.call(opera, 'Opera has modified the JavaScript on '+hostname+' (Fix rendering of HTML5 YouTube videos\nFix annotations and advance to next video on YouTube leanback...). See browser.js for details');
	...
	} else if(hostname.indexOf('56.com')>-1){			// PATCH-462, 56.com - fix missing video comments
		opera.addEventListener('BeforeScript', function(ev){
			...
			if(self==top)postError.call(opera, 'Opera has modified the JavaScript on '+hostname+' (56.com - fix missing video comments). See browser.js for details');
	...
	} else if(hostname.indexOf('capital.gr')>-1){			// PATCH-479, capital.gr: avoid unintentional XHR abort() calls
		XMLHttpRequest.prototype.abort=function(){};
			if(self==top)postError.call(opera, 'Opera has modified the JavaScript on '+hostname+' (capital.gr: avoid unintentional XHR abort() calls). See browser.js for details');
		...
		HTMLInputElement.prototype.focus=function(){};
			if(self==top)postError.call(opera, 'Opera has modified the JavaScript on '+hostname+' (Prevent focusing search field and messed up rendering). See browser.js for details');
	...
	} else if(hostname.indexOf('edition.cnn.com')>-1){			// PATCH-490, CNN: avoid plugin-triggered document.cookie setting script deadlock bug
		(function(){
			var actualGetter=document.__lookupGetter__('cookie');
			var actualSetter=document.__lookupSetter__('cookie');
			...
			if(self==top)postError.call(opera, 'Opera has modified the JavaScript on '+hostname+' (CNN: avoid plugin-triggered document.cookie setting script deadlock bug). See browser.js for details');
	...
	} else if(hostname.indexOf('fc2.com')>-1){			// PATCH-416, Fix fc2 blog editor position
		opera.addEventListener('BeforeScript',function (ev) {
			...
			if(self==top)postError.call(opera, 'Opera has modified the JavaScript on '+hostname+' (Fix fc2 blog editor position). See browser.js for details');
	} else if(hostname.indexOf('flytap.com')>-1){			// PATCH-463, Fix broken markup on FlyTap
		document.addEventListener('DOMContentLoaded', function(){ 
			...
			if(self==top)postError.call(opera, 'Opera has modified the JavaScript on '+hostname+' (Fix broken markup on FlyTap). See browser.js for details');
	...
	} else if(hostname.indexOf('hk.centamap.com')>-1){			// PATCH-318, Fix misplaced highlights on Centamap
		...
		document.addEventListener('DOMContentLoaded',function(evt){
			...
			if(self==top)postError.call(opera, 'Opera has modified the JavaScript on '+hostname+' (Fix misplaced highlights on Centamap\nFix missing menu and misplaced highlights on hk.centamap.com). See browser.js for details');
		...
		addCssToDocument("form#search-form div.searhline-outer button { text-indent:0 !important; content:'' }");
			...
			if(self==top)postError.call(opera, 'Opera has modified the JavaScript on '+hostname+' (layout issue on livedoor\'s search box). See browser.js for details');
		...
		addCssToDocument('form{opacity:1!important}');
			if(self==top)postError.call(opera, 'Opera has modified the JavaScript on '+hostname+' (orkut avatar image crop does not happen because of timing issue\nHidden form causes non-clickable bu...). See browser.js for details');
	...
	} else if(hostname.indexOf('tripadvisor.com')>-1){			// PATCH-457, Prevent double Google Maps init
		opera.addEventListener('BeforeEventListener.readystatechange', function(e){
		    if(e.event.target.src)e.preventDefault();
		}, false)
			if(self==top)postError.call(opera, 'Opera has modified the JavaScript on '+hostname+' (Prevent double Google Maps init). See browser.js for details');
	...
	} else if(hostname.indexOf('washingtonpost.com')>-1){			// PATCH-494, Washingtonpost: avoid articles being overwritten in race condition
		avoidDocumentWriteAbuse(/<div id="trc_related_container".*/);
		
			if(self==top)postError.call(opera, 'Opera has modified the JavaScript on '+hostname+' (Washingtonpost: avoid articles being overwritten in race condition). See browser.js for details');
		...
		(function(){
			...
			if(self==top)postError.call(opera, 'Opera has modified the JavaScript on '+hostname+' (Slideshare: avoid plugin-triggered document.cookie setting script deadlock bug). See browser.js for details');
	} else if(hostname.indexOf('www.ted.com')>-1){			// PATCH-450, Reading document.cookie from mouseup event that also interacts with plugin causes script deadlock
		if(navigator.platform.indexOf('Win')>-1)(function(){
			...
			if(self==top)postError.call(opera, 'Opera has modified the JavaScript on '+hostname+' (Reading document.cookie from mouseup event that also interacts with plugin causes script deadlock). See browser.js for details');
	...
	} else if(hostname=='twitter.com'){			// PATCH-444, Make Twitter hashtags visible
		addCssToDocument('a > span.hash, a > s.hash {display: inline !important} a > span.at {display: inline !important}');
			if(self==top)postError.call(opera, 'Opera has modified the JavaScript on '+hostname+' (Make Twitter hashtags visible). See browser.js for details');
